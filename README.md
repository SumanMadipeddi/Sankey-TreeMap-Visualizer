...
- Visualize a hierarchical dataset in a treemap and a flow dataset in a Sankey diagram
...
The starter code for this assignment shows three panels on the `index.html` page. The top panel contains a `textarea` html element and a `submit` button. The user can enter a set of text and then click the button. This should create a treemap chart in the bottom-left panel. The treemap will visualize the distribution of the entered text characters based on how many times they appear in the `textarea` (i.e., each rectangle will be sized based on the count of that character; a larger rectangle will indicate that character appears more often). The treemap will also be hierarchical: consonants will be one color (and grouped together), vowels another color (grouped together), and punctuation characters a third color (grouped together). 

Clicking a rectangle for a character (in the screenshot above, the `a` rectangle has been clicked) will create a Sankey diagram in the bottom-right panel. In the Sankey, the left column show chracters in the `textarea` that are immediately prior to the selected character (e.g., `la`, `ma`, `na`, etc.) and the right column characters immediately after the selected character (`am`, `ad`, `ab`, etc.). The middle column will be the selected character. All nodes in the Sankey diagram will be sized based on their count (e.g., for the left column, how often `la` occurs, how often `ma` occurs, etc.).
